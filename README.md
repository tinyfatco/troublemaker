...
The installer creates a profile in `~/.config/troublemaker/tui.json` and a command symlink in `~/.local/bin`. Keep the gateway loopback-only (or reach it through a local tunnel), and ensure `~/.local/bin` is on your `PATH`. Compact presentation is the default: the UI uses Pi's terminal styling, labels messages by source, follows the resident's live awareness stream, and shows only agent-supplied tool labels. Add `--presentation pi` during install to opt into a more Pi-faithful live tool stream. Tool calls are numbered and remain collapsed until you press Ctrl+T, release it, type a call number, and pause briefly; expanded invocation, output, and result details use the same bounded redaction projection as portable clients. Thinking and raw tool payloads never cross the live endpoint, and durable history remains compact. The UI supports `/help`, `/reload`, `/status`, `/clear`, `/stop`, and `/quit`. Input remains available during active turns so follow-ups can soft-steer the same terminal run, and automatic context compaction is labeled explicitly instead of appearing as continued thinking.
