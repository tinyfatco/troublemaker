...
The installer creates a profile in `~/.config/troublemaker/tui.json` and a command symlink in `~/.local/bin`. Keep the gateway loopback-only (or reach it through a local tunnel), and ensure `~/.local/bin` is on your `PATH`. The UI uses Pi's terminal styling, labels messages by source, follows the resident's live awareness stream, and presents tool calls as a compact numbered stream. Tool details stay collapsed by default; hold Ctrl while typing a call's number (for example, Ctrl+1 then Ctrl+2 for `[12]`) and pause briefly to expand or close its arguments, output, and result. The UI supports `/help`, `/reload`, `/status`, `/clear`, `/stop`, and `/quit`. Input remains available during active turns so follow-ups can soft-steer the same terminal run, and automatic context compaction is labeled explicitly instead of appearing as continued thinking.
