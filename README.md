...
The installer creates a profile in `~/.config/troublemaker/tui.json` and a command symlink in `~/.local/bin`. Keep the gateway loopback-only (or reach it through a local tunnel), and ensure `~/.local/bin` is on your `PATH`. Compact presentation is the default: the UI uses Pi's terminal styling, labels messages by source, follows the resident's live awareness stream, and shows only agent-supplied tool labels. Add `--presentation pi` for progressive assistant text plus bounded Pi-style tool detail, or `--presentation pi-thinking` to also paint signature-free local model reasoning as soon as each cumulative snapshot arrives. Every tool call appears as soon as its live call block arrives, updates in place while running, then settles into one summary line. A brief human-readable `label` is strongly encouraged, but omission or blank text falls back to the tool name and never blocks execution. Calls remain collapsed until you press Ctrl+T, release it, type a call number, and pause briefly; expanded invocation, output, and result details use the same bounded redaction projection as portable clients. A final call without a matching result stays visibly unknown or cancelled rather than turning green. Compact and ordinary Pi clients never receive thinking; Pi-thinking is an explicit authenticated local-terminal projection, and durable history remains compact. The UI supports `/help`, `/reload`, `/status`, `/clear`, `/stop`, and `/quit`. Input remains available during active turns so follow-ups can soft-steer the same terminal run, and automatic context compaction is labeled explicitly instead of appearing as continued thinking.
