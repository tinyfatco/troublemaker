...
assert.equal(
	mattermostFixed.workingReplyTarget,
	"mattermost:mmmmmmmmmmmmmmmmmmmmmmmmmm",
	"Mattermost fixed working output advertises its send_message-ready rollover target",
);

const zulipHarness = createHarness();
const zulip = createAdapter("zulip", (target) => createContext(target.channelId, zulipHarness));
const zulipFixed = routeWorkingOutputContext({
	policy: {
		mode: "fixed",
...
assert.equal(
	zulipFixed.workingReplyTarget,
	"zulip:4",
	"Zulip fixed working output advertises its send_message-ready rollover target",
);
