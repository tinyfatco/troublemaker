...
const server = createServer(async (req, res) => {
	...
	if (req.method === "GET" && url.pathname === "/api/v4/users/me") return json(200, { id: BOT_ID, username: "observer", is_bot: true });
	if (req.method === "GET" && url.pathname === "/api/v4/users") return json(200, [
		{ id: BOT_ID, username: "observer", is_bot: true },
		{ id: HUMAN_ID, username: "casey", first_name: "Casey", last_name: "Garcia", is_bot: false },
	]);
...
try {
	...
	broadcastPost("@observer this belongs to another private context", OTHER_CHANNEL_ID);
	...
	assert.equal(adapter.getUser(HUMAN_ID)?.userName, "casey", "allowed-channel senders load lazily");
	...
	const mentionRoot = broadcastPost("@observer please verify the migration");
	...
	const ignoredRoot = broadcastPost("ordinary work stream Observer should not continuously ingest");
	...
	assert.equal(ignoredTranscript[0]?.text, "ordinary work stream Observer should not continuously ingest", "mentions-only channels remain readable through live thread access");
	broadcastPost("@observer explicit work still wakes you");
	...
	running = true;
	broadcastPost("@observer fold this into your current work");
	...
	assert(workingPosts.some((post: any) => post.message.includes("Operator is assembling the preview")), "fixed Mattermost working output surfaces sanitized work labels");
	await assert.rejects(
