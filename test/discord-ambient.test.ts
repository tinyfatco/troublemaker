...
async function run() {
	...
	try {
		...
		await adapter.handleGatewayMessage({
			...
			author: { id: "1443881334165733499", username: "casey", global_name: "Casey" },
		...
		assert(pulse.recentMessages("1234567890123456789").length === 1, "ambient Discord message records channel pulse");
		assert(adapter.getChannel("1234567890123456789")?.name === "general", "ambient Discord message tracks channel metadata");
		assert(adapter.getUser("1443881334165733499")?.displayName === "Casey", "ambient Discord message tracks user metadata");
		...
		adapter.logBotResponse("1234567890123456789", "agent reply", "1234567890123456790");
		assert(pulse.summary("1234567890123456789").timeSinceMyLastMs < 1000, "Discord bot replies update pulse self timing");
