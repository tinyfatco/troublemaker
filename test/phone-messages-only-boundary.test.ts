...
async function run() {
	...
	try {
...

		{
			...
			await ctx.setWorking(false);
			assert.equal(sent.length, 0, "phone suppresses harness-authored runtime errors");
		}

		{
			const ts = await adapter.postMessageToRecipients("phone-test", "hello both", ["+15555550124", "+15555550124"]);
			assert.equal(ts, "phone-1", "phone explicit recipient send returns provider message id");
			assert.deepEqual(sent[0].outboundRecipients, ["+15555550123", "+15555550124"], "phone explicit recipients merge with original contact");
			assert.equal(sent[0].transport, "mms", "phone explicit recipients promote transport to mms");
