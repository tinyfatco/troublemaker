...
assert.match(runner, /SkyComputerUseClient/);
assert.match(runner, /alias: "computer-use"/);
assert.match(runner, /"sandbox"/);
assert.match(runner, /":danger-full-access"/);
assert.match(runner, /computerUseCommand, "mcp"/);
assert.match(runner, /\["peekaboo", "computer-use"\]/);
