...
const receiptBodies: Array<{ status: string; lease_token: string }> = [];
...
let releaseBatchRoute: (() => void) | undefined;
const batchRouteGate = new Promise<void>((resolve) => {
	releaseBatchRoute = resolve;
});
...
try {
	...
	const taggedDmStopResponse = await fetch(`http://127.0.0.1:${inboundAddress.port}/zulip/inbound`, {
		method: "POST",
		headers: {
			authorization: `Bearer ${INBOUND_TOKEN}`,
			"content-type": "application/json",
		...
		body: JSON.stringify({
			...stopPayload,
			deliveryId: "delivery-dm-tagged-stop",
			message: {
				...stopPayload.message,
				id: 97,
				content: '<p><span class="user-mention" data-user-id="9">@Operator</span> stop</p>',
				raw_content: "@**Operator** stop",
				flags: ["mentioned"],
			},
			hostReceipt: {
				...stopPayload.hostReceipt,
				leaseToken: "lease-dm-tagged-stop",
			},
		}),
	});
	assert.equal(taggedDmStopResponse.status, 202);

	const taggedChannelStopResponse = await fetch(`http://127.0.0.1:${inboundAddress.port}/zulip/inbound`, {
		method: "POST",
		headers: {
			authorization: `Bearer ${INBOUND_TOKEN}`,
			"content-type": "application/json",
		...
		body: JSON.stringify({
			deliveryId: "delivery-channel-tagged-stop",
			message: {
				id: 98,
				type: "stream",
				stream_id: Number(CHANNEL_ID),
			...
			hostReceipt: {
				url: `http://127.0.0.1:${upstreamAddress.port}/receipt`,
				token: RECEIPT_TOKEN,
				leaseToken: "lease-channel-tagged-stop",
			},
		}),
	});
	assert.equal(taggedChannelStopResponse.status, 202);
	for (let index = 0; index < 100 && stopped.length < 3; index++) {
		await new Promise((resolve) => setTimeout(resolve, 10));
	}
	assert.equal(stopped.length, 3, "DM and channel mentions both retain ergonomic stop behavior");
	assert.equal(stopped[1].text, "stop");
	assert.equal(stopped[1].sourceEventType, "zulip_dm");
	assert.equal(stopped[2].text, "stop");
	assert.equal(stopped[2].sourceEventType, "zulip_mention");
	assert.equal(steered.length, 0, "tagged Zulip stop commands never enter steering");
	running = false;
	...
	assert.equal(pulseRecords.length, 8, "topic mentions retain direct pulse accounting after stop controls");
	...
	for (let index = 0; index < 100 && receipts.filter((status) => status === "completed").length < 11; index++) {
		await new Promise((resolve) => setTimeout(resolve, 10));
	}
	assert.equal(receipts.filter((status) => status === "completed").length, 11);
	...
	assert.equal(pulseRecords.length, 10, "ambient and explicitly mentioned other-bot messages both reach pulse accounting");
	...
	const firstBatchResponse = await fetch(`http://127.0.0.1:${inboundAddress.port}/zulip/inbound`, {
		method: "POST",
		headers: {
			authorization: `Bearer ${INBOUND_TOKEN}`,
			"content-type": "application/json",
		...
		body: JSON.stringify({
			deliveryId: "delivery-batch-first",
			message: {
				...batchMessage,
				id: 99,
				content: "<p>Start the long review.</p>",
				raw_content: "Start the long review.",
			},
			hostReceipt: {
				url: `http://127.0.0.1:${upstreamAddress.port}/receipt`,
				token: RECEIPT_TOKEN,
				leaseToken: "lease-batch-first",
			},
		}),
	});
	...
	for (
		let index = 0;
		index < 100 && !receiptBodies.some((body) =>
			body.lease_token === "lease-batch-first" && body.status === "running"
		);
		index++
	) {
		await new Promise((resolve) => setTimeout(resolve, 10));
	}
	assert.equal(running, true, "the running receipt is emitted only after the first event enters its run route");

	const secondBatchResponse = await fetch(`http://127.0.0.1:${inboundAddress.port}/zulip/inbound`, {
		method: "POST",
		headers: {
			authorization: `Bearer ${INBOUND_TOKEN}`,
			"content-type": "application/json",
		...
		body: JSON.stringify({
			deliveryId: "delivery-batch-second",
			message: {
				...batchMessage,
				id: 100,
				content: "<p>Include the newer note too.</p>",
				raw_content: "Include the newer note too.",
			},
			hostReceipt: {
				url: `http://127.0.0.1:${upstreamAddress.port}/receipt`,
				token: RECEIPT_TOKEN,
				leaseToken: "lease-batch-second",
			},
		}),
	});
	assert.equal(secondBatchResponse.status, 202);
	for (let index = 0; index < 100 && !steered.some((event) => event.ts === "100"); index++) {
		await new Promise((resolve) => setTimeout(resolve, 10));
	}
	assert.equal(
		steered.filter((event) => event.ts === "100").length,
		1,
		"a second accepted Zulip message steers the active run instead of becoming a sequential turn",
	);
	assert.equal(
		receiptBodies.some((body) => body.lease_token === "lease-batch-first" && body.status === "completed"),
		false,
		"the first full-turn receipt remains pending while the follow-up is steered",
	);
	releaseBatchRoute?.();
	for (
		let index = 0;
		index < 100 && !receiptBodies.some((body) =>
			body.lease_token === "lease-batch-first" && body.status === "completed"
		);
		index++
	) {
		await new Promise((resolve) => setTimeout(resolve, 10));
	...
	const ambientBeforeBotDm = ambient.length;
