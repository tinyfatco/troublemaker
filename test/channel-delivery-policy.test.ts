...
try {
	...
	assert.equal(defaults.getVerbose("mmmmmmmmmmmmmmmmmmmmmmmmmm", "mattermost"), "messages-only", "Mattermost keeps its safe default without config");
	assert.equal(defaults.getVerbose("1234567890", "telegram"), "messages-only", "Telegram keeps its safe default without config");
	assert.equal(defaults.getVerbose("1234567890123456789", "discord"), "messages-only", "Discord keeps its safe default without config");
	...
	assert.equal(configured.getVerbose("1234567890", "telegram"), "messages-only", "platform-wide Telegram verbosity applies without a channel override");
	...
	assert.equal(globallyVerbose.getVerbose("1234567890", "telegram"), true, "explicit verbose default overrides Telegram messages-only");
	...
	assert.equal(isSendMessageOnlyPlatform("mmmmmmmmmmmmmmmmmmmmmmmmmm"), true, "Mattermost channel ids infer the send_message-only policy");
