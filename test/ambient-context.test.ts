...
pulse.setSelfId("UAGENT");

const channelId = "C0123456789";
const includedKeys = new Set<string>();

pulse.record(channelId, "UAGENT", "agent echo".length, "agent echo", {
	messageId: "1700000100.000001",
	threadTs: "1700000100.000001",
	replyTarget: "slack:C0123456789:1700000100.000001",
});
pulse.record(channelId, "UCASEY", "direct mention".length, "direct mention", {
	messageId: "1700000101.000001",
	threadTs: "1700000101.000001",
	replyTarget: "slack:C0123456789:1700000101.000001",
	directlyAddressed: true,
});
pulse.record(channelId, "UCASEY", "passive first".length, "passive first", {
	messageId: "1700000102.000001",
	threadTs: "1700000102.000001",
	replyTarget: "slack:C0123456789:1700000102.000001",
});
pulse.record(channelId, "UMIKE", "passive second".length, "passive second", {
	messageId: "1700000103.000001",
	threadTs: "1700000103.000001",
	replyTarget: "slack:C0123456789:1700000103.000001",
});
...
assert(firstWake.every((entry) => !entry.directlyAddressed && entry.participantId !== "UAGENT"), "first wake excludes direct mentions and agent self echoes");
...
assert(firstWake[0]?.replyTarget === "slack:C0123456789:1700000102.000001", "first wake preserves exact Slack reply target");
...
pulse.record(channelId, "UCASEY", "passive third".length, "passive third", {
	messageId: "1700000104.000001",
	threadTs: "1700000104.000001",
	replyTarget: "slack:C0123456789:1700000104.000001",
});
pulse.record(channelId, "UAGENT", "agent after wake".length, "agent after wake", {
	messageId: "1700000105.000001",
	threadTs: "1700000105.000001",
	replyTarget: "slack:C0123456789:1700000105.000001",
});
...
assert(thirdWake[0]?.text === "passive third", "later wake excludes old passive messages and fresh agent self echo");

const singleThread = resolveAmbientDeliveryContext([firstWake[0]!]);
assert(singleThread?.threadTs === "1700000102.000001", "single-message ambient wake inherits its Slack thread");
assert(singleThread?.replyTarget === "slack:C0123456789:1700000102.000001", "single-message ambient wake inherits its exact reply target");

const sharedThreadEntry = {
	...firstWake[0]!,
	messageId: "1700000102.000002",
	text: "passive follow-up",
};
const sharedThread = resolveAmbientDeliveryContext([firstWake[0]!, sharedThreadEntry]);
assert(sharedThread?.threadTs === "1700000102.000001", "multi-message ambient wake inherits one shared Slack thread");
...
const completedAfterPause = buildAmbientEvaluationText(
	"slack:#biz",
	"Casey (UCASEY): Here is the idea, gimme one sec. Let us continue after a short pause.",
...
assert(completedAfterPause.includes("gimme one sec. Let us continue after a short pause"), "ambient prompt preserves substantive content after a pause phrase");
...
stopPulse.record("CSTOP", "UCASEY", "resume old work".length, "resume old work", { messageId: "1779781000.000001" });
