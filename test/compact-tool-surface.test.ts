...
const handoff={name:"handoff_context",description:"Save a continuity checkpoint",parameters:Type.Object({summary:Type.String()})};
...
const before=compactToolContext({systemPrompt:"Stable",messages:[original],tools:[search,handoff]});
const after=compactToolContext({systemPrompt:"Stable",messages:[original],tools:[search,handoff,{name:"example_tool",description:"Large documentation".repeat(1000),parameters:Type.Object({value:Type.String()})}]});
assert.deepEqual(before,after,"tool discovery must not rewrite the provider prefix");
assert.deepEqual(before.tools?.map(tool=>tool.name),["search_tools","call_tool","handoff_context"],"compact turns keep one stable checkpoint-capable schema");
