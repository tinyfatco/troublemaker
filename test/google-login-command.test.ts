...
case "$*" in
  ...
  *"auth add jordan@example.com"*"--step 2"* )
    ...
    echo '{"stored":true,"email":"jordan@example.com","services":["calendar","contacts","docs","drive","gmail","sheets"],"client":"default"}'
    ;;
  *)
...
`,
	...
	assert.equal(resolvePendingInput("web:default", "jordan@example.com"), true);
	...
	await pending;
	assert(messages.some((message) => message.includes("Logged in to *Google Workspace* as jordan@example.com")));
