...
async function descriptorRealPath(descriptor) {
	try { return await realpath(`/proc/self/fd/${descriptor.fd}`); }
	catch (error) {
		// macOS exposes /dev/fd as descriptor nodes rather than resolvable links.
		// O_NOFOLLOW plus the lstat/fstat device+inode equality below remains the
		// portable race check; use the stronger root proof where procfs provides it.
	...
	async function walk(directory) {
		...
		for (const entry of entries) {
			...
			try {
				...
				const descriptorPath = await descriptorRealPath(descriptor);
				if (descriptorPath && !within(root, descriptorPath)) {
					throw new HostSitesError(400, "artifact_file_outside_workspace");
				}
