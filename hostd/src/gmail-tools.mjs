...
function threadAddresses(thread) {
	...
	for (const message of thread) {
		for (const field of [message.from, message.to, message.cc, message.bcc]) {
...
export class HostGmailTools {
	...
	resolveScope(target, contextId) {
		...
		if (!contact) {
			for (const route of this.store.listRoutesForContext(contextId, target.id)) {
				...
				const thread = this.gmail.getThread(route.providerThreadId);
	...
	threadForContext(target, contextId, scope, threadId) {
		...
		const thread = this.gmail.getThread(threadId);
		if (!route) {
			const external = [...threadAddresses(thread)].filter((address) => address !== this.account);
			if (external.length === 0 || external.some((address) => address !== scope.contact)) {
				throw new GmailToolError(403, "conversation_scope_denied");
			}
		}
		return thread;
	...
	search(target, contextId, input) {
		const scope = this.resolveScope(target, contextId);
		...
		const providerQuery = `(${query}) {from:${scope.contact} to:${scope.contact}}`;
		const candidates = this.gmail.searchThreads(providerQuery, Math.min(50, Math.max(limit, limit * 3)));
		const threads = [];
		for (const candidate of candidates) {
			...
			if (!route) {
				try {
					this.threadForContext(target, contextId, scope, candidate.id);
				} catch (error) {
	...
	draft(target, contextId, input) {
		...
		if (draftIdInput) {
			...
			return this.updateDraft(target, contextId, scope, idempotencyKey, draftIdInput, body);
		}
		...
		if (hasThread) {
			...
			const thread = this.threadForContext(target, contextId, scope, providerThreadId);
			const replyTarget = [...thread].reverse().find((message) => emailAddresses(message.from || "").includes(scope.contact));
		...
		let receipt;
		try {
			receipt = this.gmail.createDraft({ to: contact, subject, body, replyToMessageId });
			...
			this.verifyProviderDraft(this.gmail.getDraft(receipt.draftId), binding);
			...
			if (receipt?.draftId) {
				try {
					this.gmail.deleteDraft(receipt.draftId);
				} catch {
	...
	updateDraft(target, contextId, scope, idempotencyKey, providerDraftId, body) {
		...
		if (request.status !== "running") throw new GmailToolError(409, "draft_request_unresolved");
		try {
			const receipt = this.gmail.updateDraft(providerDraftId, {
			...
			this.verifyProviderDraft(this.gmail.getDraft(providerDraftId), next);
	...
	send(target, contextId, input) {
		...
		this.verifyProviderDraft(this.gmail.getDraft(providerDraftId), draft);
		...
		try {
			const receipt = this.gmail.sendDraft(providerDraftId);
