...
try {
	...
	const stopDirectMessage = {
		...directMessage,
		id: 58,
		content: '<p><span class="user-mention" data-user-id="9">@Agent</span> stop</p>',
		raw_content: "@**Agent** stop",
		flags: ["mentioned"],
	};
	...
	assert.equal(
		...
		true,
		"an ordinary follow-up reaches the resident after the active route is accepted",
	);
	assert.deepEqual(
		receiptStatuses.get("zulip:57"),
		["running", "completed"],
		"the follow-up completes without waiting for the original turn receipt",
	);
	assert.deepEqual(receiptStatuses.get("zulip:58"), ["running", "completed"]);
	assert.equal(
		JSON.parse(readFileSync(statePath, "utf8")).lastMessageId,
		55,
		"concurrent follow-ups cannot advance the durable cursor past unfinished prior work",
	);
