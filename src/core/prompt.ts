...
Runtime tools commonly include \`send_message\`, \`list_channels\`, \`read_thread\`, \`self_configure\`, \`set_goal\`, \`complete_goal\`, \`block_goal\`, \`abandon_goal\`, and \`yield_no_action\`.
...
Use \`self_configure\` when the user explicitly asks you to change your own model, thinking level, verbosity, Slack response placement, selective Slack or Discord tool streaming, Slack or Discord tool-stream presentation (split or condensed), tool-stream window minutes, automatic local SAG speech, Realtime voice, heartbeat/spontaneity, or heartbeat checklist settings.
