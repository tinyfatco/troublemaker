	...
	private registerTools(server: McpServer): void {
		...
		server.registerTool(
			...
			async ({ target, text, subject, attachments, recipients }: { target: string; text: string; subject?: string; attachments?: string[]; recipients?: string[] }) => {
				...
				if (!resolved) {
					return {
						content: [{ type: "text" as const, text: `No adapter found for target "${target}". Valid patterns: rocket-chat:<room>[:<root>] (Rocket.Chat), mattermost:<channel>[:<root>] (Mattermost), discord:<17-20 digit ID> or raw 17-20 digit snowflake (Discord), shorter numeric (Telegram), C/D/G prefix (Slack), slack:<channel>:<thread_ts> (Slack thread), email-thread:<id> (existing Email thread), email-{address} (Email), phone-{hash} (SMS/iMessage).` }],
						isError: true,
					};
				}
		...
		server.registerTool(
			"read_thread",
			{
				...
				inputSchema: {
					label: mcpToolLabelSchema,
					target: z.string().describe("Conversation target, e.g. rocket-chat:<room>:<root>, mattermost:<channel>:<root>, slack:C0123456789:1700000002.000100, email-thread:0123abcd..., or phone-..."),
			...
			async ({ target, limit }) => {
				...
				if (!result) {
					return { content: [{ type: "text" as const, text: `Invalid conversation target "${target}". Expected rocket-chat:<room>:<root>, mattermost:<channel>:<root>, slack:<channel>:<thread_ts>, email-thread:<id>, or phone-....` }], isError: true };
				}
