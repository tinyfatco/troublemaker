...
import { HostManagedPhoneProvider } from "./host-managed-provider.js";
...
export function createPhoneProviderRegistryFromEnv(): PhoneProviderRegistry {
	...
	const hostManaged = process.env.MOM_PHONE_HOST_MANAGED === "true";
	const hostContextId = process.env.TROUBLEMAKER_CONTEXT_ID;
	if (hostManaged) {
		if (!managedEndpoint || !managedToken || !hostContextId) {
			throw new Error("Host-managed phone delivery is missing its scoped endpoint, token, or context");
		}
		registry.register(new HostManagedPhoneProvider({
			endpoint: managedEndpoint,
			token: managedToken,
			contextId: hostContextId,
		}), true);
	} else if (managedEndpoint && managedToken) {
