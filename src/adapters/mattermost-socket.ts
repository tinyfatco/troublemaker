...
Mention users with @username.`;
	...
	constructor(config: MattermostSocketConfig) {
		...
		this.webhookOnly = config.webhookOnly ?? false;
		...
		this.deliveryLedger = new WorkspaceDeliveryLedger(
			join(this.workingDir, "mattermost-inbound-deliveries.jsonl"),
			"Mattermost delivery ledger is unreadable",
		);
	}
	...
	async start(): Promise<void> {
		...
		if (!this.webhookOnly) await this.connect();
		log.logConnected();
	}
	...
	dispatch(req: IncomingMessage, res: ServerResponse): void {
		...
		req.on("end", () => {
			...
			let payload: { deliveryId?: string; post?: MattermostPost; hostReceipt?: HostDeliveryReceipt };
			try {
			...
			if (!payload.post?.id || !payload.post.channel_id || !payload.post.user_id) {
				res.writeHead(400);
				res.end("Missing Mattermost post");
				return;
			}
			...
			void withHostReceipt(payload.hostReceipt, async () => {
				...
				await this.handlePost(payload.post!, true);
				if (payload.deliveryId) this.deliveryLedger.complete(payload.deliveryId);
			}).catch((error) => {
				log.logWarning("Mattermost webhook processing error", error instanceof Error ? error.message : String(error));
			});
		});
	}
	...
	private workspaceTransport(): WorkspaceChannelTransport {
		return {
			...
			describeReplyTarget: (_channelId, rootId) => rootId
				? "Mattermost thread containing this message"
				: "Configured Mattermost working-output destination",
		};
	}
	...
	private async handlePost(post: MattermostPost, awaitCompletion = false): Promise<void> {
		...
		if (directlyAddressed) {
			await routeWorkspaceChannelEvent({
				...
				adapter: this,
				event,
				queue: this.getQueue(post.channel_id),
				awaitCompletion,
			});
		} else {
