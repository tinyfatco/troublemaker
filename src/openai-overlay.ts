/**
 * OpenAI-specific system prompt overlay for GPT-5 models.
 *
 * Ported from OpenClaw's extensions/openai/prompt-overlay.ts (commit dfd39a81d8).
 *
 * Two layers:
 *   1. Execution bias + output contract — always applied for GPT-5 runs. Fights
 *      the "narrate the plan instead of doing it" failure mode.
 *   2. Friendly interaction style — adds warmth, emotional range, and
 *      first-person feeling language. Helps GPT-5 feel less flat.
 *
 * Scope: only applied when provider is "openai" or "openai-codex" AND the
 * model id starts with "gpt-5". Returns undefined for every other model so
 * Claude / MiniMax / Kimi / etc. get a byte-identical system prompt (preserving
...
/**
 * Returns the combined OpenAI overlay sections for GPT-5 runs, or undefined
 * for every other model. The returned string is appended to the static
 * system prompt in buildSystemPrompt().
 */
export function resolveOpenAIOverlay(model: { id?: string; provider?: string } | undefined): string | undefined {
	if (!model) return undefined;
	if (!OPENAI_PROVIDER_IDS.has(model.provider ?? "")) return undefined;
	const normalizedModelId = model.id?.trim().toLowerCase() ?? "";
	if (!normalizedModelId.startsWith(OPENAI_GPT5_MODEL_PREFIX)) return undefined;
