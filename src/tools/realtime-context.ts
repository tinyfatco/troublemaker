...
const searchSchema = Type.Object({
	query: Type.String({ description: "Case-insensitive text to search for in Agent's persisted context and chat logs." }),
	...
	return enforceRequiredToolLabels([
		{
			...
			description:
				"Return a compact briefing of Agent's identity, memory files, and recent persisted activity. Use this before answering questions that depend on Agent/Troublemaker context. This is intentionally small; use search_context for specific past-chat lookup.",
		...
		},
		{
			...
			description:
				"Search Agent's persisted awareness, adapter log, and memory files for a string. Use this when the user asks about prior chats, names, projects, decisions, or any specific term from past context.",
...
export function buildContextBriefing(
	...
	const name = stringValue(settings?.name) || stringValue(settings?.localAgentProfile) || "Agent";
