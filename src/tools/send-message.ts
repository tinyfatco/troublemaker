 ...
 *   zulip:<channel>                 -> Topic-free Zulip customer channel
...
const ZULIP_TARGET_RE = /^zulip:([1-9]\d*)$/;
...
export function resolveMessageTarget(target: string, adapters: PlatformAdapter[]): ResolvedMessageTarget | undefined {
	...
	const zulipTarget = target.match(ZULIP_TARGET_RE);
	if (zulipTarget) {
		const adapter = adapters.find((candidate) => candidate.name === "zulip");
		if (adapter) return {
			adapter,
			channel: zulipTarget[1],
			inputTarget: target,
		};
	}
...
export function createSendMessageTool(adapters: PlatformAdapter[]): AgentTool<any> {
	const schema = Type.Object({
		...
		target: Type.String({
			description: "Required destination. Examples: Telegram chat ID, Slack channel ID, slack:<channel>:<thread_ts>, mattermost:<channel>[:<root>], rocket-chat:<room>[:<root>], zulip:<channel>, discord:<channel id>, email-thread:<id>, email-user@example.com, phone-...",
		}),
	...
	return {
		...
		description:
			...
			"Target formats: discord:<17-20 digit ID> or raw 17-20 digit snowflake -> Discord, shorter numeric IDs -> Telegram, C/D/G-prefixed -> Slack, slack:<channel>:<thread_ts> -> Slack thread, mattermost:<channel>[:<root>] -> Mattermost channel/thread, rocket-chat:<room>[:<root>] -> Rocket.Chat room/thread, zulip:<channel> -> topic-free Zulip channel, email-thread:<id> -> existing Email thread, email-{address} -> Email address, phone-{hash} -> SMS/iMessage conversation. " +
		...
		parameters: schema,
			...
			if (typeof target !== "string" || !target.trim()) {
				throw new Error("send_message requires a target. Give me a destination such as rocket-chat:<room>[:<root>], mattermost:<channel>[:<root>], zulip:<channel>, slack:<channel>:<thread_ts>, email-thread:<id>, email-user@example.com, or phone-....");
			}
			...
			if (!resolved) {
				throw new Error(`No adapter found for target "${target}". Valid targets include rocket-chat:<room>[:<root>], mattermost:<channel>[:<root>], zulip:<channel>, discord:<17-20 digit ID>, raw Discord snowflake, Telegram numeric chat ID, Slack C/D/G ID, slack:<channel>:<thread_ts>, email-thread:<id>, email-{address}, or phone-{hash}.`);
			}
