...
if ! "$PLIST_BUDDY_BIN" -c "Print :EnvironmentVariables" "$CANDIDATE_PLIST" >/dev/null 2>&1; then
	"$PLIST_BUDDY_BIN" -c "Add :EnvironmentVariables dict" "$CANDIDATE_PLIST"
fi
"$PLIST_BUDDY_BIN" -c "Delete :EnvironmentVariables:TROUBLEMAKER_SOURCE_REVISION" "$CANDIDATE_PLIST" >/dev/null 2>&1 || true
"$PLIST_BUDDY_BIN" -c "Add :EnvironmentVariables:TROUBLEMAKER_SOURCE_REVISION string $ACTUAL_COMMIT" "$CANDIDATE_PLIST"
